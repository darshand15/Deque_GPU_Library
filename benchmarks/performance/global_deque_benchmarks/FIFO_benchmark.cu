...
// Kernel to push to the back of the deque
__global__ void push_back_kernel(GpuDeque<int>* d_deque, int* d_push_failures, int total_items, int work_per_thread) 
{
...
// Kernel to pop from the front of the deque
__global__ void pop_front_kernel(GpuDeque<int>* d_deque, int* d_results, int* d_pop_failures, int total_items, int work_per_thread) 
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int start_idx = tid * work_per_thread;
    ...
    for (int i = 0; i < work_per_thread; ++i) 
    {
        ...
        int popped_val = -1;
        if (d_deque->popFront(&popped_val)) 
        {
            d_results[item_idx] = popped_val;
        } 
        else 
        {
...
{
    printf("--- Verifying Deque (FIFO: push_back / pop_front) ---\n");
    
    // items range from 0 to NUM_ITEMS-1
    // hence expected sum is => (NUM_ITEMS-1) * (NUM_ITEMS-1 + 1)/2 => (NUM_ITEMS-1) * (NUM_ITEMS)/2
...
int main() 
{
    printf("--- GPU Deque Benchmark (FIFO) ---\n");
    printf("Capacity:      %d\n", CAPACITY);
    printf("Items:         %d\n", NUM_ITEMS);
    ...
    // Run FIFO Test
    {
        printf("Running FIFO Test (push_back / pop_front)...\n");
        GpuDequeHandle<int> deque_handle(CAPACITY);
        ...
        push_back_kernel<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(deque_handle.get_device_ptr(), d_push_failures, NUM_ITEMS, WORK_PER_THREAD);
        pop_front_kernel<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(deque_handle.get_device_ptr(), d_results, d_pop_failures, NUM_ITEMS, WORK_PER_THREAD);
