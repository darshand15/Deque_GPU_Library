...
// Kernel to push to the front of the deque
__global__ void push_front_kernel(GpuDeque<int>* d_deque, int* d_push_failures, int total_items, int work_per_thread) 
{
...
// Kernel to pop from the back of the deque
__global__ void pop_back_kernel(GpuDeque<int>* d_deque, int* d_results, int* d_pop_failures, int total_items, int work_per_thread) 
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int start_idx = tid * work_per_thread;
...
{
    printf("--- Verifying Deque (LIFO: pushFront / popRear) ---\n");
...
int main() {
    printf("--- GPU Deque Benchmark (LIFO) ---\n");
    printf("Capacity:      %d\n", CAPACITY);
    printf("Items:         %d\n", NUM_ITEMS);
    ...
    // Run LIFO Test
    {
        printf("Running LIFO Test (pushFront / popRear)...\n");
        ...
        push_front_kernel<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(deque_handle.get_device_ptr(), d_push_failures, NUM_ITEMS, WORK_PER_THREAD);
        pop_back_kernel<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(deque_handle.get_device_ptr(), d_results, d_pop_failures, NUM_ITEMS, WORK_PER_THREAD);
