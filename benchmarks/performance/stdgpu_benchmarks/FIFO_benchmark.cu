...
// Kernel to push to the back of the stdgpu deque
__global__ void push_back_kernel(stdgpu::deque<int> d_deque, int* d_push_failures, int total_items, int work_per_thread) 
{
...
// Kernel to pop from the front of the stdgpu deque
__global__ void pop_front_kernel(stdgpu::deque<int> d_deque, int* d_results, int* d_pop_failures, int total_items, int work_per_thread) 
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int start_idx = tid * work_per_thread;
...
{
    printf("--- Verifying stdgpu::deque (FIFO: push_back / pop_front) ---\n");
...
int main() {
    printf("--- GPU stdgpu::deque Benchmark (FIFO) ---\n");
    printf("Capacity:      %d\n", CAPACITY);
    printf("Items:         %d\n", NUM_ITEMS);
    ...
    // --- Run FIFO Test ---
    {
        printf("Running (push_back / pop_front)...\n");
        ...
        push_back_kernel<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(d_deque, d_push_failures, NUM_ITEMS, WORK_PER_THREAD);
        pop_front_kernel<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(d_deque, d_results, d_pop_failures, NUM_ITEMS, WORK_PER_THREAD);
